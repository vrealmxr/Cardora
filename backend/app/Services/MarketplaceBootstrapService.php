...
class MarketplaceBootstrapService
{
    ...
    public function build(?User $authUser, string $locale): array
    {
        ...
        $publicListings = $listingModels
            ->filter(fn (Listing $listing) => $this->marketplaceAccessService->listingIsPubliclyVisible($listing))
            ->values();
        ...
        return $userModels
            ...
            ->map(function (User $user) use ($productIdByDatabaseProductId, $productPayloadByFrontendId, $locale) {
                ...
                return [
                    ...
                    'collectionEntriesCount' => count($collectionEntryPayload),
    ...
    protected function normalizeMediaItem(mixed $item, int $index): ?array
    {
        if (is_string($item) && $item !== '') {
            ...
            $thumbUrl = $this->resolvePreviewStorageUrl($item, 'public') ?? $resolvedUrl;
        ...
        $thumbUrl = $this->normalizePublicUrl(is_string($item['thumb_url'] ?? null) ? $item['thumb_url'] : null)
            ?? $this->normalizePublicUrl(is_string($item['preview_url'] ?? null) ? $item['preview_url'] : null)
            ?? $this->resolvePreviewStorageUrl(is_string($item['preview_path'] ?? null) ? $item['preview_path'] : $path, $disk)
            ?? $url;
    ...
    protected function resolvePreviewStorageUrl(?string $path, string $disk = 'public'): ?string
    {
        ...
        $previewPath = preg_replace('/\.webp$/i', '-preview.webp', $normalizedPath);
        if (! is_string($previewPath) || $previewPath === $normalizedPath) {
            return null;
        }

        return Storage::disk($disk)->exists($previewPath)
            ? $this->storageUrl($previewPath, $disk)
            : null;
    }
